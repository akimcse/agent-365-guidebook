...
| v6 | 2026-07-20 | 부록 '데모 시나리오'(demo-scenarios.html) 신규 추가 — Agent 365 위협 탐지를 4계층(콘텐츠·행위·데이터·신원)별로 검증하는 참조 페이지(계층×플랫폼 탐지 매핑, CS×2·Foundry×2 커스텀 에이전트, 레이어별 시나리오). 전 페이지 네비에 부록 그룹 통일 | — |
| v7 | 2026-07-20 | 심화 문서③ '데이터 보안 · Purview' 신규 추가(기존 런타임 위협 탐지는 ④로 이동) — 분류 엔진(SIT·Trainable Classifier·EDM)/라벨링/정책(DLP·Communication Compliance·IRM)/DSPM 집계·Audit·eDiscovery 계층 아키텍처, 위험 유형별 엔진 매핑, 포털 경로, Purview 고유 vs 非M365 PAYG 경계. 전 페이지 네비 ③④ 재배치 | — |
